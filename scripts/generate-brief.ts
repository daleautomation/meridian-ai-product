...
async function main() {
  ...
  const isSample = args.get("sample") === "true";
  const disclaimer = args.get("disclaimer") ?? (isSample ? DEFAULT_SAMPLE_DISCLAIMER : undefined);

  if (!customer || !csvPath) {
    throw new Error("Usage: npx tsx scripts/generate-brief.ts --customer=test --csv=fixtures/sample-recovery.csv [--sample=true]");
  }
  ...
  const brief: RecoveryBrief = {
    ...
    ...(isSample ? { isSample: true, sampleDisclaimer: disclaimer } : {}),
    summary: {
  ...
  console.log(`Opportunities: ${brief.summary.opportunities}${isSample ? " (SAMPLE)" : ""}`);
}
