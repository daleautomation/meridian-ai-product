...
import { normalizeExportRow } from "../lib/recovery/normalize";
import {
  formatContactPath,
...
function inferModuleFromIndustry(industry: string | null): string {
  ...
  return "roofing"; // safe default — only affects the NormalizedLead branding
}
...
async function buildItem(row: CsvRow, index: number, now: Date): Promise<RecoveryBriefItem> {
  ...
  const lead = normalizeLead({
    ...
    crm: {
      ...
      lastAction: r.lastNote ?? undefined,
      notes: r.lastNote ?? undefined,
    },
  }, {
  ...
  // Only honor a founder-authored opportunityLabel. The scoring engine's
  // decision.primaryOpportunity is roofing-specific and leaks "Local SEO
  // visibility" into staffing/B2B briefs — don't let it reach the memo.
  ...
  // Only carry the opportunity into the brief output when it was authored
  // for this row. Scoring-engine defaults (roofing-trained) do not transfer
  // cleanly to staffing/B2B verticals.
