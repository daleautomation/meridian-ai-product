...
function inferModuleFromIndustry(industry: string | null): string {
  ...
  return "roofing"; // safe default — only affects NormalizedLead branding, not output text
}
...
async function buildItem(row: CsvRow, index: number, now: Date): Promise<RecoveryBriefItem> {
  ...
  // Realism guard: a vague note ("vm", "followed up", "no response") is
  // technically present but carries no business reason. Drop it at the
  // generator boundary so it never gets quoted back as if substantive.
  const usableNote = isVagueNote(r.lastNote) ? null : r.lastNote;
  const usableNextStep = isVagueNote(r.nextStep) ? null : r.nextStep;
  ...
  const lead = normalizeLead({
    ...
    signals: {
      hasWebsite: r.website ? true : undefined,
      recentActivity: r.recentActivity ?? undefined,
    },
    crm: {
      status: r.status ?? undefined,
      lastAction: usableNote ?? undefined,
      notes: usableNote ?? undefined,
    },
  }, {
  ...
  const priorityContext = generatePriorityRead({
    ...
    nextStep: usableNextStep,
    activityLabel: r.activityLabel,
    // Roofing-trained decision.primaryOpportunity is deliberately not used here
    // to prevent "Local SEO visibility" leak into staffing/B2B briefs.
