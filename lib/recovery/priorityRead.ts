...
//
// Replaces dashboard "N/100 fit with M/100 staleness" framing with 1–2
// sentences of operator reasoning. Quality-tiered to avoid invented
// specificity on weak data.
...
export function generatePriorityRead(input: PriorityReadInput): string {
  ...
  switch (angle.source) {
    ...
    case "note": {
      // Trim quoted notes to a single sentence or ~90 chars so the priority
      // read stays one prescriptive line, not a paragraph of operator copy.
