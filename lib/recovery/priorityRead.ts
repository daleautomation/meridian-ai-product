...
//
// Replaces the dashboard-style "80/100 fit with 90/100 staleness" framing
// with one to two sentences of operator reasoning:
//   - why this lead is worth action this week (timing + signal)
//   - what to lead with (angle from the strongest available signal)
//   - optionally, one narrow question to ask
//
// Pure derivation from already-classified inputs. No new scoring.
...
export type PriorityReadInput = {
  ...
  activityLabel?: string | null;       // founder-authored boost
  opportunityLabel?: string | null;    // founder-authored or derived
  priorityNote?: string | null;        // founder-authored boost
  crmStatus?: string | null;
  dealStage?: string | null;
  lifecycleStage?: string | null;
  industry?: string | null;
  hasVerifiedContactPath?: boolean;
  companyName?: string | null;         // used to vary phrasing across cards
};
...
export function generatePriorityRead(input: PriorityReadInput): string {
  // Honor a founder-authored priorityNote when present — that text was
  // written specifically for this row. Otherwise compose from signals.
  ...
  // Compose a "lead with" clause that varies by angle source so the line
  // doesn't read identically across cards.
