...
import { firstSentenceOf, type DataQuality } from "@/lib/recovery/normalize";
...
function whyWorthIt(input: PriorityReadInput, seed: string): string {
  ...
  if (note && typeof days === "number" && days >= 30) {
    return ["the prior note carried a concrete reason rather than a vague interest signal", "the last exchange ended with something specific enough to pick up again", "the prior thread had a real ask attached to it, not just curiosity"][v];
  }
  if (next && typeof days === "number" && days >= 30) {
    return ["the open next-step they outlined never closed out", "the action they committed to is still sitting unresolved", "their stated next move never got executed and the door is still open"][v];
  }
  if (input.staleCategory === "Recovery candidate") {
    return ["the account is in the window where threads either restart or quietly die", "this is the recovery zone — past polite, not yet cold", "the relationship is at the inflection point between dormant and lost"][v];
  }
  if (input.staleCategory === "Dormant") {
    return ["the relationship is dormant but the prior context is still usable", "the account has cooled, but the prior thread can still carry the call", "it is dormant, not dead — the original context still applies"][v];
  }
  if (input.hasVerifiedContactPath) return "the contact path is verified and there is no active obstacle to a direct re-entry";
...
export function generatePriorityRead(input: PriorityReadInput): string {
  ...
  if (note) return note.endsWith(".") || note.endsWith("?") || note.endsWith("!") ? note : `${note}.`;
  ...
  if (!angle.source || !angle.label) return `${action} because ${reason}.`;

  let leadClause: string;
  switch (angle.source) {
    case "boost": leadClause = ` Lead with the change you flagged — ${lowerLead(angle.label)} — and let that carry the reason for the call.`; break;
    case "opp": leadClause = ` Lead with the ${lowerLead(angle.label)} angle; that is the cleanest reason to revisit the relationship.`; break;
    case "next": leadClause = ` Lead with the action you'd outlined — to ${lowerLead(angle.label)} — and ask whether it should still happen.`; break;
    case "note": {
      const short = firstSentenceOf(angle.label, 90) ?? angle.label;
      leadClause = ` Lead with the prior note ("${short}") and treat this as a continuation, not a restart.`;
      break;
    }
    case "stage": leadClause = ` Lead with the ${angle.label.toLowerCase()} conversation as it stood, narrower this time.`; break;
  }
