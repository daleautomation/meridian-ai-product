...
//
// 8-branch signal hierarchy. Variant phrasing. Quality-tiered: LOW data
// produces calm narrow output instead of inventing context.
...
function isQualifiedStatus(value: string | null | undefined): boolean {
  ...
  return ["qualified", "open", "proposal", "proposal_sent", "demo", "demo_booked", "interested", "warm", "in_discussion", "negotiation", "in_negotiation", "follow_up", "follow_up_needed"].includes(n);
}
...
function staleClause(days: number | null | undefined, variant: number): string {
  ...
  if (days < 30) return variant === 0 ? " The window is still short enough to keep the conversation continuous." : " It's been a couple of weeks — close enough to pick up the thread.";
  if (days < 60) return variant === 0 ? ` It has been ${days} days, so a narrow check-in is welcome, not intrusive.` : ` ${days} days have passed — long enough to be useful, not long enough to feel cold.`;
  if (days < 120) return variant === 0 ? ` ${days} days of silence is exactly the window where these threads quietly die.` : ` After ${days} days, the relationship is past polite and into recovery range.`;
  return variant === 0 ? ` The relationship has been quiet for ${days} days; the prior thread is the most credible way back in.` : ` ${days} days is long enough that a direct, specific reopen reads better than a soft touch.`;
}
...
export function generateWhyNow(input: WhyNowInput): string {
  ...
  if (quality === "low") {
    ...
    if (input.hasVerifiedContactPath) return "Reachable contact on file with limited recent history. Worth a single, exploratory check-in this week.";
  ...
  if (input.hasVerifiedContactPath) return "Reachable contact on file with no recent outreach logged. Worth a single, specific check-in this week.";
