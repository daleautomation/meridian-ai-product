...
//
// Produces a short, founder-voiced opening line. Three-part shape:
//   1. Greeting (first name when available)
//   2. Anchor (prior thread when available, OR a timing cue, OR a
//      conservative low-data line — never invent a hook)
//   3. One narrow question with a concrete verb
//
// Hard-banned phrases (filter retries with a neutral fallback if any
// of these slip through via a CSV-authored note):
...
//   - HIGH quality (note or next-step present): lead with that thread
//   - MEDIUM quality (status + dates only): reopen at the prior stage
//   - LOW quality (sparse): a short, conservative, exploratory line
//                          that explicitly does not pretend to know more
...
function isQualifiedStatus(value: string | null | undefined): boolean {
  ...
  return [
    "qualified", "open", "proposal", "proposal_sent", "demo", "interested",
    "warm", "in_negotiation", "follow_up",
  ].includes(n);
}
...
function composeBody(input: OpenerInput, variantSeed: string): string {
  ...
  // LOW-QUALITY GUARD: do not invent a hook. Stay narrow and honest.
  if (quality === "low") {
    const variants = [
  ...
  // 2) Last note → reference the open thread.
  if (note) {
  ...
  // 4) Recent activity on their side while we paused.
  ...
  // 7) Medium-quality days-only fallback.
