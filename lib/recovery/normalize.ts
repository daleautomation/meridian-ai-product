...
//
// Resolves CRM export columns to a logical row regardless of casing,
// punctuation, or alias variation. Hardened against null-likes, partial
// phones, malformed dates, malformed booleans, merged contact fields,
// and operator-edited spreadsheets.
//
...
// ── Meta-note detection ──────────────────────────────────────────────
// Operators routinely use the notes field for *meta-commentary* about
// data state rather than actual customer conversation:
//   "No specific note on file"
//   "No contact named — was sourced from a directory"
//   "Was previously routed through a partner contact who has since left"
// These must NOT be templated as customer speech ("you no specific note…").
// Detected and dropped at the generator boundary, same as vague notes.
...
// ── Verb-start check ─────────────────────────────────────────────────
// The "you ___" opener template only reads grammatically when the note
// starts with a recognizable past-tense action verb. Returns the cleaned
// note when it leads with one of these verbs; otherwise null so the
// generator can fall back to a safer construction.
const LEADING_VERBS = new Set([
  "asked", "wanted", "requested", "paused", "mentioned", "spoke", "said",
  "talked", "agreed", "confirmed", "shared", "raised", "flagged",
  "expressed", "noted", "indicated", "wanted", "pushed", "declined",
...
// Notes that start with an action verb ("Asked about X") but later
// contain "they"/"their"/"said" are operator commentary referring to the
// customer in third person. Don't template them as direct customer speech.
...
export function containsThirdPersonTail(value: string | null | undefined): boolean {
  ...
  const tail = tokens.slice(1).join(" "); // skip the leading verb
  return THIRD_PERSON_TAIL.test(tail);
}

// First-sentence extraction for keeping priority-read quotes short.
export function firstSentenceOf(value: string | null | undefined, maxChars = 90): string | null {
  ...
  if (candidate.length > maxChars) {
    // Cut at the nearest space before maxChars.
...
export function classifyDataQuality(row: NormalizedExportRow): DataQuality {
  const noteUsable = !!row.lastNote && row.lastNote.length >= 10
    && !isVagueNote(row.lastNote) && !isMetaNote(row.lastNote);
  const stepUsable = !!row.nextStep && row.nextStep.length >= 6
    && !isVagueNote(row.nextStep) && !isMetaNote(row.nextStep);
