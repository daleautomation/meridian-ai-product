Dylan, the hard thing you are probably avoiding is sending the Recovery Brief offer to one real buyer and letting the answer constrain what gets built next.

## Evidence Audited

- Repository state: active branch `cursor/founder-challenge-brief-ff90`; HEAD `a19b063 feat: add career calendar sync`; `main` and `origin/main` point to the same commit.
- Current branch delta: `git log main..HEAD` is empty. This branch began as a brief branch but has no unique product commits before this file.
- Uncommitted work before this brief: `package-lock.json` only, deleting 105 optional `@next/swc-*` package entries. The install script ran `npm install`, reported 5 vulnerabilities, and removed 1 package. This is environment/package-lock churn, not revenue work. It remains unstaged.
- Existing founder brief on this branch before this file: none under `research/strategy/`; only `.gitkeep` existed.
- Founder brief history: remote Founder Brief branches exist for June 10, 14, 15, 17, 18, 19, 20, 21, 22, and 23. Open PRs still include prior founder brief branches #74, #75, #76, #77, #78, and #79. The brief cadence exists, but it is fragmented across unmerged branches instead of becoming durable operating state on `main`.
- Current heartbeat run: `npm run heartbeat:run` on 2026-06-25 passed 7/7 observer-safe checks and generated `generated/heartbeat/latest.md` plus `generated/heartbeat/brief-today.md`.
...
- CRM audit evidence: `data/crmActivities.json` records internal site opens, domain opens, scans, and contact searches across roofing companies. It does not record sent outreach, replies, meetings, pilots, payment, or revenue.
- CRM import evidence: `data/crmImportJobs.json` has one previewing test job, one row, and 0 imported rows.
- Review evidence: `data/reviews.json` is empty.
- Existing review artifacts: `.github/pull_request_template.md` and `docs/workflows/pr-review-checklist.md` require trust, explainability, commercial prioritization, noise reduction, and no AI theater. Open PRs include multiple prior founder-brief PRs and relationship/recovery/system work, but no PR evidence of customer pull.
...
- The public homepage sells "Founder-reviewed weekly Recovery Briefs" and routes "See a sample brief" to the staffing sample.
- The product canon says the core build is weekly Recovery Briefs for dormant relationships, not a broad operating platform.
- The outreach admin page says the recommended first vertical is boutique staffing and recruiting firms.
