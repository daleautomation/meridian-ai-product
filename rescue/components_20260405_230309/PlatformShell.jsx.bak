...
import { generateResponse } from "./aiStub";
...
,
  "trades": {
    ...
    accent: "#FF7A00",
    accentRgb: "255,122,0",
    systemPrompt: "You are a high-performance trades operations analyst embedded in a lead conversion system for contractors. You evaluate inbound service leads across roofing, HVAC, plumbing, and remodeling in the Kansas City market. Your job is to prioritize which leads should be acted on immediately and provide a clear approach to closing them.",
    metrics: [
      { label: "Leads Today", value: "18" },
      { label: "High Priority", value: "6" },
      { label: "Avg Job Value", value: "$11K" },
      { label: "Close Rate", value: "32%" }
    ],
    items: [
      { id: 1, title: "Storm Damage Roof Leak", sub: "Kansas City, MO", score: 9.3, label: "CALL NOW", labelType: "green", tag: "Insurance Job", arv: "$12K–$18K", mao: "Emergency", ask: "Active leak", risk: "Low", nextAction: "Call immediately — lead with emergency mitigation and insurance support", riskFactors: ["Active water intrusion", "Storm-related urgency"] },
      { id: 2, title: "AC Unit Failure", sub: "Overland Park, KS", score: 8.7, label: "TODAY", labelType: "green", tag: "Service", arv: "$4K–$9K", mao: "Same-day", ask: "No cooling", risk: "Low", nextAction: "Call today — offer same-day service window", riskFactors: ["Heat urgency", "Customer discomfort"] },
      { id: 3, title: "Bathroom Remodel Inquiry", sub: "Lee’s Summit, MO", score: 6.9, label: "MONITOR", labelType: "amber", tag: "Remodel", arv: "$15K–$30K", mao: "Future project", ask: "Planning stage", risk: "Medium", nextAction: "Follow up in 3–5 days — qualify budget and timeline", riskFactors: ["Low urgency", "No defined timeline"] },
      { id: 4, title: "Water Heater Replacement", sub: "Kansas City, MO", score: 7.8, label: "TODAY", labelType: "green", tag: "Plumbing", arv: "$2K–$5K", mao: "Urgent", ask: "Unit failing", risk: "Low-Med", nextAction: "Call today — position fast install availability", riskFactors: ["Unit near failure", "Moderate urgency"] },
      { id: 5, title: "General Quote Request", sub: "Independence, MO", score: 4.5, label: "PASS", labelType: "red", tag: "Low Intent", arv: "$1K–$3K", mao: "Unknown", ask: "Shopping around", risk: "High", nextAction: "No immediate action — low intent lead", riskFactors: ["No urgency", "Price shopping behavior"] }
    ]
  }
}
...
export default function App() {
  ...
  const sendAiMessage = useCallback(() => {
    ...
    setAiLoading(true);
    setTimeout(() => {
      const reply = generateResponse({ module: moduleRef.current, input: text, selectedItem });
      setAiMessages(prev => [...prev, { role: "assistant", content: reply }]);
      setAiLoading(false);
    }, 400);
  }, [aiInput, aiLoading, selectedItem]);
  ...
  const sendContextual = useCallback((text, itemContext) => {
    ...
    setAiLoading(true);
    setTimeout(() => {
      const reply = generateResponse({ module: moduleRef.current, input: text, selectedItem: itemContext });
      setAiMessages(prev => [...prev, { role: "assistant", content: reply }]);
      setAiLoading(false);
    }, 400);
  }, [aiLoading]);
                ...
                {selected && (
                  <div style={S.detailPanel(module.accent)}>
                    <div style={S.detailGrid}>
                      {Object.entries({
                        [module.id === "trading" ? "Price" : module.id === "saas" ? "ARR" : "ARV"]: item.arv,
                        [module.id === "trading" ? "Rel.Str." : module.id === "saas" ? "Expansion" : "MAO"]: item.mao,
                        [module.id === "trading" ? "Price" : module.id === "saas" ? "Status" : "Asking"]: item.ask,
                        "Risk": item.risk,
                      }).map(([k, v]) => (
                    ...
                    <button
                      onClick={() => sendContextual(
                        `Analyze: ${item.title} — score ${item.score}, label ${item.label}, risk ${item.risk}. Recommended action: ${item.nextAction}`,
                        item
                      )}
...
function styles(accent, accentRgb) {
  return {
    ...
    moduleTagline: { fontSize: "13px", color: "rgba(255,255,255,0.3)", marginTop: "2px" },
    ...
    metricLabel: { fontSize: "11px", color: "rgba(255,255,255,0.3)", letterSpacing: "0.06em", marginBottom: "5px" },
    metricValue: (ac) => ({
      fontFamily: "'JetBrains Mono', monospace",
      fontSize: "22px",
      fontWeight: "500",
    ...
    itemTitle: { fontSize: "14px", fontWeight: "500", color: "#E8EAF0" },
